...
////unsigned char CAN1_Init(CO_Data * d, uint32_t bitrate)			//replaced by static void MX_FDCAN1_Init(void),whu
////{
...
//////    res = CAN_Transmit(CANx, &TxMessage);		//stm32f4xx_can.c
////	
...
unsigned char canSend(CAN_PORT notused, Message *msg)
{
	...
	//while(( HAL_CAN_AddTxMessage(&hcan,&TxHeader,msg->data,&TxMailbox)!=HAL_OK) || time > 200)
		while(( HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1,&TxHeader,msg->data)!=HAL_OK) || time > 200)
...
//replaced by FDCAN1_IT0_IRQHandler,whu
